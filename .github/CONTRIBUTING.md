...
2. **Validate devcontainer-feature.json files** (`validate.yml`) — JSON schema validation, shellcheck on shell scripts, and README sync check.
3. **Lint workflows** (`lint-workflows.yml`) — `actionlint` on every workflow file.
4. **Conventional Commits** (`conventional-commits.yml`) — PR title lint.
...
2. When a bump is needed, it opens a pull request titled `chore: bump feature versions`.
3. After that PR merges to `main`, `tag-release.yml` creates any missing prefixed tags from the current JSON versions.
4. Pushing a prefixed tag triggers `release.yaml`, which publishes the feature to GHCR.
