...
if [ "$ENFORCE_SAFETY" = "true" ]; then
    ...
    check "00-human-sovereignty.md absent" test ! -f "$RULES_DIR/00-human-sovereignty.md"
    check "00-no-attribution.md absent" test ! -f "$RULES_DIR/00-no-attribution.md"
    check "00-no-secrets.md absent" test ! -f "$RULES_DIR/00-no-secrets.md"
fi
...
if [ "$STANDARDIZE_WORKFLOW" = "true" ]; then
    ...
    check "00-mcp-tools-first.md absent" test ! -f "$RULES_DIR/00-mcp-tools-first.md"
    check "00-skill-discovery.md absent" test ! -f "$RULES_DIR/00-skill-discovery.md"
    check "04-anti-overengineering.md absent" test ! -f "$RULES_DIR/04-anti-overengineering.md"
    check "01-conventional-commits.md absent" test ! -f "$RULES_DIR/01-conventional-commits.md"
    check "03-branch-strategy.md absent" test ! -f "$RULES_DIR/03-branch-strategy.md"
fi
...
if [ "$PROTECT_GIT" = "true" ]; then
    ...
    check "00-no-git-config-override.md absent" test ! -f "$RULES_DIR/00-no-git-config-override.md"
fi
...
if [ "$PREFER_PYTHON_TOOLING" = "true" ]; then
    ...
    check "00-prefer-uv.md absent" test ! -f "$RULES_DIR/00-prefer-uv.md"
    check "01-markdown-formatting.md absent" test ! -f "$RULES_DIR/01-markdown-formatting.md"
fi
